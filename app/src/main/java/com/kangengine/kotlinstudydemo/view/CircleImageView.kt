    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (rawBitmap != null && mType != TYPE_NONE) {
            ...
            if (mType == TYPE_CIRCLE) {
                ...
                mRectBorder.set(halfBorderWidth,halfBorderWidth,
                    dstWidth - doubleBorderWidth,dstHeight - halfBorderWidth)
                mRectBitmap.set(0.0f,0.0f,dstWidth-doubleBorderWidth,dstHeight-doubleBorderWidth)
                val borderRadius = if(mRectRoundRadius - halfBorderWidth > 0.0f)
                    mRectRoundRadius - halfBorderWidth else 0.0f
                val bitmapRadius = if (mRectRoundRadius - mBorderWidth > 0.0f)
                    mRectRoundRadius - mRectRoundRadius else 0.0f
